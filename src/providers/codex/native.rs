...
use super::translate::model_allowlist::{
    MODEL_ALIASES, allowed_models_display, assert_allowed_model, fast_model_base,
    full_lane_web_search_model, uses_responses_lite,
};
...
fn resolve_native_model(requested: &str) -> (String, bool) {
    let (requested, priority) = match fast_model_base(requested) {
        Some(base) => (base, true),
